...
plot_meteo <- function(data,columna,titulo, unidades)
{
  library(nycflights13)
  weather <- nycflights13::weather
  EWR1 <- weather[(weather$origin=="EWR"),]
  LGA1 <- weather[weather$origin =="LGA",]
  JFK1 <-weather[weather$origin=="JFK", ]
  dat <- data.frame(data)
  x <-dat[,columna] 
  boxplot(x ~ dat$month, ylab = unidades, xlab= "Months",col="pink", main = titulo)
  m <-mean(x[columna])
  EWR_media<- mean(dat[,columna], na.rm = T)
...
```
He tenido que añadir un argumento más a la función para especificar el origen que quiero que me muestre, en caso de no querer que muestre un origen en concreto pondría cualquier otra palabra que no fuera el nombre de alguno de los aeropuertos y mostraría el boxplot de todo el data weather.
